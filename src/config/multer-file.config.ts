...
const formDataMiddleware = (req: Request, res: Response, next: NextFunction) => {
    ...
    singleFileUpload(req, res, async (error: multer.MulterError | any) => {
        ...
        if (error instanceof multer.MulterError) {
            ...
            if (error.code === 'LIMIT_FILE_SIZE') {
                ...
                return responseError(res, 400, 'Field file is required!');
            } else if (error.code === 'LIMIT_FILE_COUNT') {
                return responseError(res, 400, 'Only one file allowed! Please select only one file.');
            }
        } else if (error) {
            // Handle other errors
